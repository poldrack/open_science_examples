...
results_sim_full['params'] = {'int_param_max': int_param_max,
                            ...
                            'n': n,
                            'formulas': [formula, formula_multisite, formula_summary]}
...
for i in range(nruns):
    ...
    results_sim.loc[i, 'int_pval_simple'] = sm3_coefs[9,4]
...
out_path = Path('../processed_data/powersim')
if not out_path.exists():
    out_path.mkdir()

results_sim.to_csv(str(out_path) + '/power_sim_mixedmodel1_%s.csv' % run_string)
with open(str(out_path) + '/power_sim_mixedmodel1_%s.pkl' % run_string, 'wb') as f:
